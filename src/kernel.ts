  ...
  call(...args: TArgs): void {
    for (const tap of this.taps) {
      try {
        ...
        throw new PluginError(tap.name, error instanceof Error ? error.message : String(error))
      }
    }
  }
}

export class AsyncSeriesHookImpl<TArgs extends any[]> implements AsyncSeriesHook<TArgs> {
...
export class AsyncSeriesWaterfallHookImpl<TArgs extends any[], TResult>
  implements AsyncSeriesWaterfallHook<TArgs, TResult>
{
  private taps: Array<{
    name: string
    fn: (...args: [...TArgs, (result: TResult) => void]) => Promise<void>
  }> = []
  ...
  async callAsync(...args: TArgs): Promise<TResult> {
    let result: TResult | undefined

    for (const tap of this.taps) {
      try {
        await tap.fn(...args, (value) => {
          result = value
        })
      } catch (error) {
        throw new PluginError(tap.name, error instanceof Error ? error.message : String(error))
...
export class AsyncParallelHookImpl<TArgs extends any[]> implements AsyncParallelHook<TArgs> {
  private taps: Array<{ name: string; fn: (...args: TArgs) => Promise<void> }> = []
...
export class KernelImpl<TContext = BundlerContext> implements Kernel<TContext> {
  ...
  hooks: BundlerHooks = {
    ...
    transform: new AsyncSeriesWaterfallHookImpl<[TransformArgs], TransformResult>(),
    renderChunk: new AsyncSeriesWaterfallHookImpl<[RenderChunkArgs], RenderChunkResult>(),
  ...
  use(plugin: Plugin<TContext> | (() => Plugin<TContext>)): this {
    ...
    if (this.plugins.has(instance.name)) {
      throw new PluginError(instance.name, `Plugin "${instance.name}" is already registered`)
    }
  ...
  unregister(name: string): void {
    ...
    if (plugin.onDestroy) {
      try {
        ...
        throw new PluginError(name, error instanceof Error ? error.message : String(error))
      }
    }
    ...
    for (const [name, plugin] of Array.from(this.plugins.entries()).reverse()) {
      if (plugin.onDestroy) {
        try {
          ...
          if (plugin.onError) {
            plugin.onError(error instanceof Error ? error : new Error(String(error)))
          }
        }
      }
    }
